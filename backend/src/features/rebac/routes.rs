...
use super::models::*;
use super::impact::{ImpactService, SimulateRoleChangeInput, ImpactReport};
...
pub fn rebac_routes() -> Router<RebacService> {
    Router::new()
        // Permission types
        // User scoped roles
        .route("/users/:user_id/roles", get(list_user_roles).post(assign_role))
        ...
        .route("/permission-types", get(list_permission_types).post(create_permission_type))
        .route("/permission-types/:id", put(update_permission_type).delete(delete_permission_type))
        // Relationship Types CRUD
        .route("/relationship-types", get(list_relationship_types).post(create_relationship_type))
        .route("/relationship-types/:id", put(update_relationship_type).delete(delete_relationship_type))
        // Role permission management
        .route("/roles/:role_id/permissions", get(get_role_permissions))
        .route("/roles/:role_id/permission-mappings", get(get_role_permission_mappings))
        .route("/roles/:role_id/permissions/:permission", post(add_role_permission).delete(remove_role_permission))
        ...
        // Delegation Rules
        .route("/delegation-rules", get(list_delegation_rules).post(add_delegation_rule))
...
async fn get_access_matrix(
    ...
    svc.get_active_user_roles_batch(input.user_ids).await
        .map(Json)
        .map_err(|e| {
...
async fn list_user_roles(
    ...
    svc.list_user_scoped_roles(user_id).await
        .map(Json)
...
async fn assign_role(
    ...
    svc.assign_scoped_role(input, granter_id).await
        .map(Json)
        .map_err(|e| {
...
async fn revoke_role(
    ...
    svc.revoke_scoped_role(id, None, input.reason).await
...
async fn update_role_schedule(
    ...
    svc.update_role_schedule(id, input.schedule_cron).await
        .map(Json)
        .map_err(|e| {
...
async fn check_permission(
    ...
    let result = svc.check_permission(
        query.user_id, 
        query.entity_id, 
        &query.permission,
        query.tenant_id,
    ...
    let response = PermissionCheckResponse {
        ...
        reason: if result.is_denied.unwrap_or(false) {
            ...
            format!("Granted via role '{}'{}",
                result.granted_via_role.unwrap_or_default(),
                if result.is_inherited.unwrap_or(false) { " (inherited)" } else { "" }
            )
        } else {
...
async fn check_bulk_permissions(
    ...
    let results = svc.check_multiple_permissions(
        payload.user_id, 
        payload.entity_ids, 
        &payload.permission,
        payload.tenant_id
    ...
    let response = BulkCheckResponse {
        results: results.into_iter().map(|(id, allowed, denied)| EntityPermissionResult {
            entity_id: id,
            allowed,
            is_denied: denied,
        }).collect(),
...
async fn get_entity_permissions(
    ...
    svc.get_user_entity_permissions(query.user_id, entity_id).await
        .map(Json)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn get_accessible_entities(
    ...
    svc.get_accessible_entities(query.user_id, &perm_query.permission).await
        .map(Json)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn get_role_permission_mappings(
    ...
    svc.get_role_permission_mappings(role_id).await
        .map(Json)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn get_role_permissions(
    ...
    svc.get_role_permissions(role_id).await
        .map(Json)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn add_role_permission(
    ...
    svc.add_permission_to_role(role_id, &permission, query.field_name).await
...
async fn remove_role_permission(
    ...
    svc.remove_permission_from_role(role_id, &permission).await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|_| StatusCode::NOT_FOUND)
}
...
async fn validate_cron(
    Json(input): Json<ValidateCronRequest>,
...
async fn list_permission_types(
    ...
    svc.list_permission_types().await
        .map(Json)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn create_permission_type(
    ...
    svc.create_permission_type(input).await
...
async fn update_permission_type(
    ...
    svc.update_permission_type(id, input).await
        .map(Json)
        .map_err(|_| StatusCode::NOT_FOUND)
}
...
async fn delete_permission_type(
    ...
    svc.delete_permission_type(id).await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|_| StatusCode::NOT_FOUND)
}
...
async fn create_relationship_type(
    ...
    svc.create_relationship_type(input).await
...
async fn update_relationship_type(
    ...
    svc.update_relationship_type(id, input).await
        .map(Json)
        .map_err(|_| StatusCode::NOT_FOUND)
}
...
async fn delete_relationship_type(
    ...
    svc.delete_relationship_type(id).await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|_| StatusCode::NOT_FOUND)
}
...
async fn update_role_level(
    ...
    svc.update_role_level(id, input.level).await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn list_delegation_rules(
    ...
    svc.list_delegation_rules(None).await
        .map(Json)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn remove_delegation_rule(
    ...
    svc.remove_delegation_rule(id).await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|_| StatusCode::NOT_FOUND)
}
...
async fn simulate_role_change(
    ...
    impact_svc.simulate_role_change(input).await
        .map(Json)
        .map_err(|e| {
